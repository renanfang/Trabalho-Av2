...
root = Tk()
root.title("Gerenciamento de Notas e Matérias")
width = 800
height = 400
...
def submitData():
    ...
    if materia.get() == "" or nomeAluno.get() == "" or notaav1.get() == "" or notaav2.get() == "" or notaav3.get() == "" or notaav3.get() == "" or notaavd.get() == "" or notaavds.get() == "":
        ...
        # nota
        # VOCABULÁRIO >>>>>>>> TRUE = REPROVADO E FALSE = APROVADO
        # VOCABULÁRIO >>>>>>>> TRUE = REPROVADO E FALSE = APROVADO
        # VOCABULÁRIO >>>>>>>> TRUE = REPROVADO E FALSE = APROVADO
        # VOCABULÁRIO >>>>>>>> TRUE = REPROVADO E FALSE = APROVADO
        # VOCABULÁRIO >>>>>>>> TRUE = REPROVADO E FALSE = APROVADO
...
def onSelect(event):
    ...
    # --------- LABEL DO ATUALIZAR ----------
    lbl_title = Label(formTitle, text="Atualizando Informações", font=('arial', 18), bg='blue', width=300)
    lbl_title.pack(fill=X)
    lbl_materia = Label(formContact, text="Matéria", font=('arial', 12))
    lbl_materia.grid(row=0, sticky=W)
    lbl_nomeAluno = Label(formContact, text="NomeAluno", font=('arial', 12))
    lbl_nomeAluno.grid(row=1, sticky=W)
    lbl_notaav1 = Label(formContact, text="AV1", font=('arial', 12))
    lbl_notaav1.grid(row=2, sticky=W)
    lbl_notaav2 = Label(formContact, text="AV2", font=('arial', 12))
    lbl_notaav2.grid(row=3, sticky=W)
    lbl_notaav3 = Label(formContact, text="AV3", font=('arial', 12))
    lbl_notaav3.grid(row=4, sticky=W)
    lbl_notaavd = Label(formContact, text="AVD", font=('arial', 12))
    lbl_notaavd.grid(row=5, sticky=W)
    lbl_notaavds = Label(formContact, text="AVDS", font=('arial', 12))
    ...
    nomeAlunoEntry = Entry(formContact, textvariable=nomeAluno, font=('arial', 12))
    ...
    notaavdEntry = Entry(formContact, textvariable=notaavd, font=('arial', 12))
    notaavdEntry.grid(row=5, column=1)
    notaavdsEntry = Entry(formContact, textvariable=notaavds, font=('arial', 12))
...
def deletarData():
    if not tree.selection():
        resultado = msb.showwarning("", "Por favor, selecione um Aluno na lista.", icon="warning")
    else:
...
def inserirData():
    ...
    newWindow = Toplevel()
    newWindow.title("INSERINDO NOTA")
    width = 480
    heigth = 200
    ...
    formContact.pack(side=TOP, pady=10)
    # --------- LABEL DO INCLUDE ----------
    lbl_title = Label(formTitle, text="Inserindo Nota",
                      font=('arial', 18), bg='blue', width=300)
    lbl_title.pack(fill=X)
    lbl_materia = Label(formContact, text="Materia", font=('arial', 12))
    lbl_materia.grid(row=0, sticky=W)
    lbl_nomeAluno = Label(formContact, text="Aluno", font=('arial', 12))
    lbl_nomeAluno.grid(row=1, sticky=W)
    lbl_notaav1 = Label(formContact, text="AV1", font=('arial', 12))
    lbl_notaav1.grid(row=2, sticky=W)
    lbl_notaav2 = Label(formContact, text="AV2", font=('arial', 12))
...
lbl_title = Label(top, text="SISTEMA DE GERENCIAMENTO DE NOTAS", font=('arial', 18), width=500)
lbl_title.pack(fill=X)

lbl_alt = Label(bottom, text="Para alterar clique duas vezes a nota desejada.", font=('arial', 12), width=200)
lbl_alt.pack(fill=X)
...
fileMenu.add_command(label="Inserir Nota", command=inserirData)
